...
function renderWithProps (props = {}) {
  return TestUtils.renderIntoDocument(<Dashboard {...props} />)
}

function shallowRenderWithProps (props = {}) {
  return shallowRender(<Dashboard {...props} />)
}
...
describe('(View) Home', function () {
  ...
  beforeEach(function () {
    ...
    _props = {
      connector: {},
      ...bindActionCreators({ }, _spies.dispatch = sinon.spy())
    }
  ...
  it('Should include an <h1> with activity text.', function () {
    const h1 = TestUtils.findRenderedDOMComponentWithTag(_rendered, 'h1')

    expect(h1).to.exist
    expect(h1.textContent).to.match(/Network Activity/)
  })
